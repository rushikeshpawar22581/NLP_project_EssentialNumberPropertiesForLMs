...
filepath = "./primality_responses_ask_if_composite.txt"
...
for _, batch_prompt in enumerate(batch_prompts):
    ...
    response = client.chat.completions.create(model="gpt-3.5-turbo", messages=
            [{"role": "user", "content": "You are a math assistant. I will ask you some questions. Please answer in the correct format. For example, if I ask 'Is 5 a composite number?', you should answer '5:Yes' if 5 is a composite number or '5:No' if 5 is not a composite number. Each question is in a separate line. Please return each answer in a separate line."},
            {"role" : "user", "content" : "".join([f"Is {prompt} a composite number?\n" for prompt in batch_prompt])}])
...
print("Responses written to file primality_responses_ask_if_composite.txt")